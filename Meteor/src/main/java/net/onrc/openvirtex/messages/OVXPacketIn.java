...
public class OVXPacketIn extends OVXMessage implements Virtualizable {
    ...
    public void virtualize(final PhysicalSwitch sw) {
    	...
    	//TrafficCounter opc = OpenVirteXController.getInstance().makeCounter();
        ...
        if (this.port.isEdge()) {
            ...
            if (this.tenantId == null) {
                ...
                //this.log.info("1TrafficCounter`s info: {}", opc.getLastReadThroughput());
                //this.log.info("1----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
                return;
            }
            ...
            this.log.debug("Edge PacketIn {} sent to virtual network {}", this.getOFMessage().toString(),
                    this.tenantId);
            //this.log.info("2TrafficCounter`s info: {}", opc.getLastReadThroughput());
            //this.log.info("2----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
            return;
        }
        ...
        if (match.get(MatchField.ETH_TYPE) == EthType.IPv4
            ...
            if (lUtils.isValid()) {
                ...
                if (srcPort == null) {
                    ...
                    //this.log.info("3TrafficCounter`s info: {}", opc.getLastReadThroughput());
                    //this.log.info("3----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
                    return;
                }
                ...
                try {
                	...
                	//this.log.info("4TrafficCounter`s info: {}", opc.getLastReadThroughput());
                	//this.log.info("4----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
                ...
                if (linkField == OVXLinkField.MAC_ADDRESS) {
                    ...
                    //this.log.info("5TrafficCounter`s info: {}", opc.getLastReadThroughput());
                    //this.log.info("5----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
                    return;
                }
            }
            ...
            if (match.get(MatchField.ETH_TYPE) == EthType.ARP) {
                ...
                this.installDropRule(sw, match);
                //this.log.info("6TrafficCounter`s info: {}", opc.getLastReadThroughput());
                //this.log.info("6----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
                return;
            }
            ...
            this.log.info("IPv4 PacketIn {} sent to virtual network {}", this,
                    this.tenantId);
            //this.log.info("7----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
            return;
        }
        ...
        if (this.tenantId == null) {
            this.log.debug(
                    ...
                    this);
            this.installDropRule(sw, match);
            //this.log.info("8TrafficCounter`s info: {}", opc.getLastReadThroughput());
            //this.log.info("8----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
            return;
        }
        vSwitch = this.fetchOVXSwitch(sw, vSwitch, map);
        ...
        this.log.info("Layer2 PacketIn {} sent to virtual network {}", this.getOFMessage(),
                this.tenantId);
        //this.log.info("9TrafficCounter`s info: {}", opc.getLastReadThroughput());
        //this.log.info("9----- TrafficCounter`s info read: {} / wirte: {} / lastwirte: {}", opc.getCumulativeReadBytes(), opc.getCurrentWrittenBytes(), opc.getLastWrittenBytes());
    }
