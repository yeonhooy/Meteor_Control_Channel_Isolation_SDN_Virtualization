...
public abstract class OVXSwitch extends Switch<OVXPort> implements Persistable {
    ...
    //Sincon integer;
    public static long[][] cumulative_throughputs = new long [100][300];
    public static int[][] message_counts = new int [100][300];
    public static long[][] average_throughput = new long [100][300];
    public static int[][] initial_Check = new int[100][300];
...
//        this.log.info("setActive = " + isActive);
    ...
    public boolean boot() {
        ...
        //this.roleMan.datacollect_start();
        return true;
    }
    ...
    public void sendMsg(final OVXMessage msg, final OVXSendMsg from) {
        ...
        if (pair != null) {
            ...
            c = pair.getSwitch(); //return channel
        }
        //log.info("XXX SwitchID: {}/({})- What is xid: {} - msgType {}",this.switchName,this.tenantId,pair.getXid(),msg);
        //log.info("XXX msgType {}",msg.getOFMessage().getType());

        if (this.isConnected && this.isActive) {
        	String swName = this.getSwitchName().substring(21);
        	int swDec = Integer.parseInt(swName,16);
        	int tId = this.tenantId;
        	long curruentThroughput = 0;
        	curruentThroughput = roleMan.sendMsg(msg.getOFMessage(), c, tId, swDec);
        	//per Message LOG
        	//log.info("SwitchID: {}/({})-MessageType: {} >> {} ",this.switchName,this.tenantId, msg.getOFMessage().getType(), currentThroughput);
            //countResult = roleMan.sinconString(gct);
            if(countResult != " "){
            	//log.info("SwitchID: {}/({})-MessageType: {} >> {} ",this.switchName,this.tenantId, msg.getOFMessage().getType(), countResult);
            }else{
            	//log.error("SwitchID: {}/({})-No Sincon String result",this.switchName,this.tenantId);
            }
        } else {
    ...
    public void handleRoleIO(OVXMessage msg, Channel channel) {
        ...
        if(msg.getOFMessage().getVersion() == OFVersion.OF_10) {
            ...
            try {
                ...
                log.info("Finished handling role for {}/s>{}:t>{}",
                        channel.getRemoteAddress(),this.getSwitchName(),this.getTenantId());
